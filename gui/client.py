...
class cvStreamClient:
    ...
    def cvStreamConnect(self):
        ...
        #         self.videoStream()
        #     except:
        #         print("Couldn't connect to the CAMERA server... Reconnecting in 1 sec")
        #         # self.clientCV.close()
        #         time.sleep(1)
        ...
        while True:
            while len(self.data) < self.payload_size:
                try:
                    ...
                    print(f"\rrec frame\t<---\t{self.fc}\n", end="")
                    self.fc += 1
                    if not packet:
                        print("Error receiving initial packet...")
                        continue
                    self.data += packet
            ...
            if frame is not None:
                try:
                    # self.drawHUD(frame)
                    self.frame_queue.put(frame, block=True, timeout=None)
                    # return frame 
