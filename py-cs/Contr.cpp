...
Contr::Contr(Cam* cam)
	: cam(cam), acceptor(ioService, ip::tcp::endpoint(ip::tcp::v4(), 11111)),
...
{
	//range->getPacket();
	//std::cout << range->dist << " meters." << std::endl << std::endl;
...
void Contr::operating()
{
	...
	std::thread cams(&Contr::recording,this);
	std::thread comm(&Contr::listening,this);
	
	
	if(cams.joinable()){cams.join();}
	...
	//std::future<void> cams = std::async(std::launch::async, &Contr::recording, this);
	//std::future<void> comm = std::async(std::launch::async, &Contr::listening, this);
